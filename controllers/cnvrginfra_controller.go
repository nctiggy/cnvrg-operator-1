...
func (r *CnvrgInfraReconciler) infraNetworkingState(infra *mlopsv1.CnvrgInfra) error {
	...
	if err := r.FixIstioDeploymentError(infra); err != nil {
		return err
	}
	return nil
}
...
func (r *CnvrgInfraReconciler) FixIstioDeploymentError(cnvrgInfra *mlopsv1.CnvrgInfra) error {
	if true || cnvrgInfra.Spec.Networking.EastWest.Enabled && cnvrgInfra.Spec.Networking.EastWest.Primary == false && cnvrgInfra.Spec.Networking.EastWest.ClusterIpsEnabled {
		infraLog.Info("running istio deployment fix if there is immutable error")
		ctx := context.Background()
